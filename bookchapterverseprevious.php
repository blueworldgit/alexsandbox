...
// DEBUG LOGGING
echo "<script>console.log('PREVIOUS.PHP DEBUG:');</script>";
echo "<script>console.log('SESSION db_type:', '" . $_SESSION['db_type'] . "');</script>";
echo "<script>console.log('POST db_type:', '" . (isset($_POST['db_type']) ? $_POST['db_type'] : 'NOT SET') . "');</script>";

// Access the global database config to see what we're connected to
global $current_db;
if (isset($current_db)) {
    echo "<script>console.log('Current DB Config:', " . json_encode($current_db) . ");</script>";
}
...
echo "<script>console.log('PREVIOUS.PHP: Using bibletable:', '" . $bibletable . "');</script>";
...
$query = mysqli_query($conn, "SELECT * FROM `{$bibletable}` WHERE `keyid` = '{$keyvalue}' LIMIT 1");

// DEBUG: Log what table and database we're querying
echo "<script>console.log('PREVIOUS.PHP QUERY DEBUG:');</script>";
echo "<script>console.log('bibletable:', '" . $bibletable . "');</script>";
echo "<script>console.log('Database name:', '" . (function_exists('get_db_display_name') ? get_db_display_name() : 'unknown') . "');</script>";
...
<?php if($_SESSION['db_type'] == 'english') { 
    // English database layout - same as verse.php
...
?>
	...
	$queryid_english = mysqli_query($conn, "SELECT * FROM `{$english_methodtable}` WHERE `verseno` = '{$keyvalue}' LIMIT 1");
...
<?php } else { ?>
	<!-- Hebrew/Greek Database Section - original layout -->
        <div class="row">
		  <div class="col-md-8">
